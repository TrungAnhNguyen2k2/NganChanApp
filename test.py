...
def run_powershell_command(command):
    # Chạy PowerShell với câu lệnh được cung cấp
    process = subprocess.Popen(command, shell=True)

    # Chờ cho PowerShell hoàn thành
    process.wait()

    # Nếu PowerShell trả về mã lỗi, in ra lỗi
    if process.returncode != 0:
        print(f"PowerShell trả về mã lỗi: {process.returncode}")
        return
if os.name == 'nt':
...
def main(argv):
    ...
    if msg.command == ADD_COMMAND:
        ...
        # Chạy câu lệnh "taskkill /f /im "notepad.exe"
        run_powershell_command("taskkill /f /im notepad.exe")
